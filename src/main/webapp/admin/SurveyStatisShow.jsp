...
for(Question q:qlist){
	...
	if(q.getQType()==5){
		...
		<tr><td><%=bodys[j] %>:</td><td><img src="images/blue.gif" width="<%=Integer.valueOf(results[j])*100.00/total*3%>" height="20"></td><td><%=results[j] %>/<%=total %>&nbsp;&nbsp;<%=df.format(Integer.valueOf(results[j])*100.00/total)%>%</td><td><%=(q.getQType()==3||q.getQType()==4)&&j==bodys.length-1?"<a href=ShowText.jsp?qid="+q.getQId()+" target=_blank>查看具体</a>":"" %></tr>
