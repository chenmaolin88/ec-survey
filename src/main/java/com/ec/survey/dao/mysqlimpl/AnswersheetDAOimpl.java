package com.ec.survey.dao.mysqlimpl;
...
public class AnswersheetDAOimpl implements AnswersheetDAO{
	...
	public boolean isIpRepeate(Long sid, String IP) {
		SQLCommand cmd =new SQLCommand();
		RowSet rs=cmd.queryRowSet("select s_id from answersheet where s_id="+sid+" and as_userip='"+IP+"'");
		try {
			if(rs.next()){
				return true;
			}else
