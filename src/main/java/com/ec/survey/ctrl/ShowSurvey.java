...
public class ShowSurvey extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if(ipRepeatSetting==false&&isIpRepeat==true){
			response.sendRedirect(request.getContextPath()+"/OpResult.jsp?op=default&ret=false&words="+URLEncoder.encode("操作失败，该问卷设置了禁止同一IP重复提交，您的IP已经过提交问卷！", "UTF-8"));
			return;
		}
		...
		if(success==true) {
			...
			response.sendRedirect(request.getContextPath()+"/OpResult.jsp?ret=true&op=ShowSurvey");}
		 
		else
			response.sendRedirect(request.getContextPath()+"/OpResult.jsp?ret=false&op=ShowSurvey");
	}
	}
