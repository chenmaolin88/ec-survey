	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("AddAdmin".equals(op)){
			...
			if(ret1)
				...
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=default&ret=false&words="+URLEncoder.encode("增加管理员出错！请联系管理员", "UTF-8") );
		}
			...
			if(ret1)
				...
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=default&ret=false&words="+URLEncoder.encode("删除管理员出错！请联系管理员", "UTF-8"));
			...
			if(dao.checkPwd(username, oldpwd)!=true){
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=default&ret=false&words="+URLEncoder.encode("旧密码错误", "UTF-8"));
				return;
			}
			...
			if(ret1)
				...
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=default&ret=false&words="+URLEncoder.encode("编辑管理员出错！请联系管理员", "UTF-8"));
			}
		}
	}
