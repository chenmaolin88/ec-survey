...
public class SurveyManage extends HttpServlet {
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if("AddSurvey".equals(request.getParameter("op"))){
			...
			if(ret==true)
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=SurveyAdd&ret=true");
			else
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=SurveyAdd&ret=false");
			...
			if(ret==true)
				response.sendRedirect(request.getContextPath()+"/admin/SurveyAudi.jsp");
			else
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=SurveyAudi&ret=false");
			}
			...
			//System.out.println(ret);
			if(ret==true)
				response.sendRedirect(request.getContextPath()+"/admin/SurveyEdit.jsp?sid="+sid+"&words="+URLEncoder.encode("操作成功！", "UTF-8"));
			else
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=SurveyEdit&ret=false");
			...
			if(ret1==true)
				response.sendRedirect(request.getContextPath()+"/admin/SurveyAdmin.jsp");
			else
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=SurveyDel&ret=false");
		}
