...
public class StatisManage extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
			...
			if("DelSheet".equals(op)){
				...
				if(ret==true)
					response.sendRedirect(request.getContextPath()+"/admin/ShowSheets.jsp?sid="+sid);
				else
					response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=DelSheet&ret=false");
				...
				synchronized(this){
					...
					if(success==true)
						response.sendRedirect(request.getContextPath()+"/admin/SurveyStatis.jsp?words="+URLEncoder.encode("操作成功!", "UTF-8"));
					else
						response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?ret=false&op=default&words="+URLEncoder.encode("问卷统计出错,请与系统管理员联系！","UTF-8"));
				}
			}
	}
