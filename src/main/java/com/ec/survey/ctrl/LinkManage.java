...
public class LinkManage extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("AddLink".equals(op)){
			...
			if(ret1)
				response.sendRedirect(request.getContextPath()+"/admin/FriendLinkAdmin.jsp");
			else
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=default&ret=false&words="+URLEncoder.encode("增加友情链接出错！请联系管理员", "UTF-8"));
		}
			...
			if(ret1)
				...
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=default&ret=false&words="+URLEncoder.encode("删除友情链接出错！请联系管理员", "UTF-8"));
			...
			if(ret1)
				...
				response.sendRedirect(request.getContextPath()+"/admin/OpResult.jsp?op=default&ret=false&words=编辑友情链接出错！");
			}
		}
	}
